...
{
printk(KERN_INFO "push latency eka printti");
    int wp, rp;
    ...
    irqgen_data->wp = wp;
    irqgen_data->rp = rp;
printk(KERN_INFO "push latency vika printti");
}
...
printk(KERN_INFO "handler eka printti");
...
# endif
    ...
    // {{{ CRITICAL SECTION
	spin_lock_irqsave(&irqgen_data->lock, irqgen_data->flags); // irqspin lock
    ++irqgen_data->total_handled;
    ++irqgen_data->intr_handled[idx];
printk(KERN_INFO "handler ennen push latency printti");
    irqgen_data_push_latency(idx, latency, timestamp);
printk(KERN_INFO "handler jäkeen push latency printti");
    // }}}
...
static int irqgen_probe(struct platform_device *pdev)
{
	...
	// spinlock
	//	spin_lock_irq(&irqgen_data->lock);
    ...
    DEVM_KZALLOC_HELPER(irqgen_data->intr_ids,
                        pdev, irqs_count, GFP_KERNEL); // devm_kfree
	...
	// spin_unlock_irq(&irqgen_data->lock);
    return 0;
...
//	int i = 0;
//	for 
//    devm_free_irq(&irqgen_data->intr_idx[i])
...
MODULE_AUTHOR("Juho Kylväjä <juho.kylvaja@tuni.fi>");
MODULE_AUTHOR("Mikko Saari <mikko.I.saari@tuni.fi>");
MODULE_VERSION("0.7");
